...
{
    EXPORT_LOG(__FUNCTION__);
	return context.OnGPUdmaChain(baseAddrL, addr);
}
...
// GTE
void CALLBACK EXPORT_GTEfifoInvalidate(u32 cmd)
{
	EXPORT_LOG(__FUNCTION__);
	return context.OnGteFifoInvalidate(cmd);
}

void CALLBACK EXPORT_GTEfifoAdd(s64 llx, s64 lly, s16 z)
{
	EXPORT_LOG(__FUNCTION__);
	return context.OnGteFifoAdd(llx, lly, z);
}

void CALLBACK EXPORT_GTEtransferToRam(u32 address, u32 cmd)
{
	EXPORT_LOG(__FUNCTION__);
	return context.OnGteTransferToRam(address, cmd);
}

void CALLBACK EXPORT_GTEwriteDataMem(u32* pMem, s32 size, u32 address)
{
	EXPORT_LOG(__FUNCTION__);
	return context.OnGTEwriteDataMem(pMem, size, address);
}
